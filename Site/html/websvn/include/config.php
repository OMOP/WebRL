...
if (!isset($_SESSION['OMOP_Auth']) 
	|| !isset($_SESSION['OMOP_Auth']['has_svn_access']) 
	|| $_SESSION['OMOP_Auth']['has_svn_access'] != 'Y')
{
//	var_dump($_SESSION);
    ...
    $svn_sub_folder = $user->organization->organization_svn_folder;
    if (!$svn_sub_folder)
        $svn_sub_folder = "/";
}
$config->allowDownload();
/** End OMOP Integration patch ********************/
...
// $config->addRepository('NameToDisplay', 'URL to repository (e.g. file:///c:/svn/proj)');
 //$config->addRepository('omop', 'file:///var/repos/omop');
...
//
//$config->addRepository('omop', 'http://174.129.41.64/svn');

// Display Part of a repository as if it was a repository.
//
// Local repositories (without and with optional group):
//
$config->addRepositorySubpath('omop', 'file:///var/repos/omop', $svn_sub_folder);
...
//
//$config->parentPath('/var/repos/','omop');
...
//$config->useAuthenticationFile('/etc/httpd/dav_svn.passwd');
$config->useAuthenticationFile('/var/repos/omop/conf/svnserve.conf','omop');
